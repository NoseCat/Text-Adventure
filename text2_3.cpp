	...
	for (int wait = 0, loot_table = 1, loot_bunch = 1; wait <= 3;)
	{
		...
		case 2:
			...
			if (loot_bunch > 0)
			{
				...
				system("pause");
				Player cockroach;
				cockroach.max_health = 100.f;
				cockroach.health = 100.f;
...
void clown_2_3_2()
{
	...
	do
	{
		...
		case 4:
			system("cls");
			sh_continue = event_progulka_2_3();
			break;
		default:
...
int event_progulka_2_3()
{
	...
	case 2:
		...
		else
		{
			...
			if (mirror == 0)
			{
				printprint("В рабочем столе оказалось пару полезных вещей.\n(+10 к деньгам, теперь у вас есть необычное зеркало)\n", 15);
				_player_.money += 10;
				mirror = 1;
				system("pause");
			}
			...
			if (sharp_2_3 == 0)
			{
				...
				printf("Вы закончили осматривать кабинет.\n");
				system("pause");
			}
	...
	case 4:
		...
		{
			printprint("\nВам не хваитит ни на что денег, Вы уходите.\n", 15);
			system("pause");
			system("cls");
			break;
		}
	...
	do {
		...
		case 3:
			if (cabinet == 0)
			{
				...
				printprint("Вы заканчиваете осматривать кабинет и уходите.\n", 15);
				system("pause");
				system("cls");
...
void Bonk_2_3_3()
{
	...
	system("pause");
	system("cls");
	printprint("Вас отводят к дрессировщику и отныне он Ваш босс.\n", 15);
	...
	//MiDiN: 1. Они были 0, потому что я считаю что 0 = false. 2. Потому что есть два варианта одного и того же действи: по человечески и как псина сутулая. 
	//3. Скорее всего ничего не поменяется, но тебе теперь год придётся ждать проверки, до субботы не успеешь.
	do
	{
		...
		case 3:
			if (!x35)
			{
				...
				system("cls");
				printprint("Вы подходите к выделенному Вам «жилому» углу.\nСобачья конура будет более «жилой», впрочем, может Вас в неё позже и переселят.\n", 15);
				if (mirror == 1)
				{
		...
		case 5:
			if (!x35)
			{
				...
				system("cls");
				printprint("Вы подползаете к выделенному Вам «жилому» углу.\n", 15);
		...
		case 7:
			if (!x17)
			{
				...
				{
					printprint("Цирковые силачи недоверчиво смотрят на вас, но решают не придавать значения и пытаются покормить вас с руки.\n(+1 к волкоподобности)\n", 15);
					auf += 1;
					system("pause");
					system("cls");
				}
