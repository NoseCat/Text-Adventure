...
void opening_of_the_circus_2_3_0()
{
	...
	if (was_at_circus == 1)
	{
		printprint("Вы уже пробовали работать в цирке, независимо от результата, теперь это завершённая часть вашей истории.", 15);
		system("pause");
		system("cls");
		Begining();
	}
	...
	printprint("\n1) Продолжить. (2 и более харизмы)\n2) Не решится. (попробовать найти другой способ заработать)\n", 15);
	printstat(_player_, charisma); printf("\n");
	short choice1 = choice(2);
	if (choice1 == 1 and _player_.charisma > 1)
		beginning_of_the_circus_2_3_0();
	else
		if (choice1 == 1)
		{
			printprint("Вы не решаетесь рисковать и идти на это страшное испытание - проходить собеседование.", 15);
			system("pause");
			system("cls");
			Begining();
			return;
...
void beginning_of_the_circus_2_3_0()
{
	...
	if (_player_.charisma < 3)
		printprint("Ваша харизма показала персоналу цирка, что вы не способны сделать практически ничего самостоятельно. Вас отвели к «главному».\n", 15);
	else
	{
		printprint("Ваша харизма показала персоналу цирка, что вы вполне самостоятельный. Вы самостоятельно пошли к «главному».\n", 15);
		circus_reputation++;
	}
	printprint("\n По пути вы наблюдаете за репетицией цирковой труппы.\n", 15);
	switch (rand() % 3 + 1)
	{
	case 1:
	...
	case 2:
		printprint("Вы наблюдали за трюковым номером волков. ( +1 к волкоподобности)", 15);
		auf++;
		break;
	...
	case 3:
		...
		default:
			printf("\nswitch error: beginning_of_the_circus_2_3_0() floor2\n");
			system("pause");
			break;
		}
		break;
...
void meet_with_antrepreneur_2_3_0()
{
	...
	for (int wait = 0, loot_table = 1, loot_bunch = 1; wait <= 3;)
	{
		...
		case 1:
			...
			if (loot_table > 0)
			{
				...
				printprint("В столе нету ничего, что стояло бы Вашего внимания.", 15);
			system("pause");
			break;
			// Веши на полу
		case 2:
		...
		case 5:
			...
			switch (wait)
			{
				// +1 wait
			case 0:
				printprint("Теперь вы бездельничаете снаружи... ", 15);
				Sleep(1000);
			...
			case 1:
				...
				case 1:
					...
					{
						printprint("Вы неловко мямлите. Парень спешит пройти мимо...", 15);
						system("pause");
						wait++;
						break;
					}
			...
			case 3:
				...
				case 1:
					...
					printprint("\n(-\b+\b-5\b6\b5 о\bО\bот\bТ\bт р\bР\bре\bЕ\bеп\bП\bпу\bУ\bут\bТ\bта\bА\bац\bЦ\bци\bИ\bии\bИ\bи в\bВ\bв ц\bЦ\bци\bИ\bир\bР\bрк\bК\bке\bЕ\bе)", 15);
	...
	printprint("\nНекто: «Не буду тянуть волка за хвост и представлюсь. Я - антрепренёр данного цирка.»\n", 15);
	system("pause");
...
void dialog_with_antrepreneur()
{
	...
	case 4:
		...
		/*case 4: 4) Вы пришли сюда не шутки шутить, а зарабатывать серьёзные деньги.\n
			printprint("Антрепренёр: «Вот как.", 15);
			Sleep(1500);
...
void circus_choice_2_3(int num)
{
	...
	{
	case 1:
		printprint("Вы решаете пойти по трудоёмкому, но незамысловатому пути. От того видите вы здесь плюс и минус или два минуса будет зависеть остаток вашей жизни...\n", 15);
		system("pause");
		system("cls");
...
void defender_2_3_1()
{
	...
	system("cls");
	printprint("На следующий день атрепренёр вызывает Вас к себе. Он обьясняет Ваши обязанности.\nКак оказалось эта работа довольно лёгкая. Всё что от вас требуется:\nохранять вещи антрепренёра от тараканов. По какой-то причине обычный охрынник ему не нужен...\n", 15);
	printprint("Антрепренёр уходит по своим делам, оставляя вас на посту. Не потребовалось много времени,\nчтобы в поле зрения появился первый таракан - отныне ваш естественный враг.\n", 15);
	system("pause");
	system("cls");
	fight(cockroach);
	system("cls");
	...
	else
	{
		...
		case 3:
			system("cls");
			printprint("Вы выходите из цирка, но чувствуете сильную боль в животе, в нём торчит похожая на волчью лапа.\nВаше приключение на этом заканчивается.\n", 15);
			death();
			system("pause");
			break;
	...
	system("pause");
	system("cls");
	printprint("Вы работаете «охранником» цирка на протежении многих лет.\nПока одним вечером на вас не нападает огромный волкоподобный монстр. Истекая кровью вы вспоминаете,\nчто незадолго до вашей необычной находки, пропало несколько циркачей,\nа ещё одного через месяц нашли в виде костей в лесу\n", 15);
...
void clown_2_3_2()
{
	...
	case 3:
		...
		else if (_player_.strength >= 40)
		{
			...
			{
				printprint("После выступления к вам подходит мускулистый мужчина и сильно хлопает вас по плечу.", 15);
	...
	do
	{
		...
		case 2:
			...
			if (mirror == 1)
			{
				...
				case 2:
					system("cls");
					printprint("Вы долго смотрите в зеркало...", 15);
...
void event_progulka_2_3()
{
	...
	case 5:
		...
		if (choice(2) == 2)
			break;
		system("cls");
		printprint("Вы наткнулись на стаю волков.", 15);
...
void oddity_event_2_3()
{
	...
	case clown:
		...
		case 1:
			...
			{
				printprint("Он предлагает Вам стать вам оборотнем.\n1) Согласиться\n2) Отказаться\n", 15);
				if (choice(2) == 1)
					ending_2_3(6);
			}
			else
		...
		case 2:
			...
			if (sherlok > 10)
			{
				...
				{
					system("cls");
					printprint("Вы находите антрепренёра. После обвинения его в пропаже циркачей вы начинаете бой.", 15);
					final_fight_2_3();
				}
			}
			else
			{
				printprint("Вам не приходит в голову идей, что щас происходит в цирке.\nСледовательно у вас есть два варианта:\n1) Уйти с тем, что уже есть.\n2) Продолжить, не обращая внимания на окружающий сюр.", 15);
				if (choice(2) == 1)
				{
					system("cls");
					ending_2_3(1);
	...
	do
	{
		...
		case 3:
			if (x35 == 0)
			{
				...
				printprint("(+2 к волкободобности)", 15);
				auf += 2;
				if (mirror == 1)
				{
